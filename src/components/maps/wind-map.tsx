...
const REGION_OUTLINE: FeatureCollection = (() => {
  ...
  for (const f of REGION.features) {
    const g = f.geometry;
    if (!g) continue;
    if (g.type === "Polygon") for (const r of g.coordinates) lines.push(r);
    else if (g.type === "MultiPolygon")
      for (const p of g.coordinates) for (const r of p) lines.push(r);
  }
  const feat: Feature = {
    type: "Feature",
    properties: {},
    ...
    <div className={cn("@container", bare ? "flex h-full w-full flex-col" : "space-y-3")}>
      ...
      {data && (
        <p className="px-3 text-[10px] text-neutral-500 sm:px-0">
          ...
          }).format(new Date(data.generatedAt))} · Quelle: MeteoSchweiz ICON-CH1 — stündliche Windböen 10 m, +0 … +24 h
