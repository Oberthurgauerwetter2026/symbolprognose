  ...
  return (
    <div className="space-y-2">
      <div className="relative overflow-hidden rounded-md">
        <div
          ref={scrollerRef}
          className="flex gap-px bg-zinc-200 border border-zinc-200 rounded-md overflow-x-auto touch-pan-x overscroll-x-contain snap-x snap-mandatory no-scrollbar"
        >
          {days.map((day, i) => {
          ...
          return (
            ...
            >
              ...
              <div className="flex flex-col items-center text-center">
                <span
                  className={`text-base @[640px]:text-lg font-bold font-[family-name:var(--font-display)] ${
                  selected ? "text-accent" : "text-zinc-900"
                }`}
                >
                  {i === 0 ? "Heute" : i === 1 ? "Morgen" : weekdayLong(day.date)}
                </span>
                <span className="text-xs @[640px]:text-sm text-zinc-700 font-semibold">
                  {formatDateShort(day.date)}
                </span>
              </div>
              <div
                className="py-0.5 select-none text-zinc-900 flex justify-center [&_svg]:h-12 [&_svg]:w-12 @[640px]:[&_svg]:h-14 @[640px]:[&_svg]:w-14"
              ...
              <div className="pt-1 border-t border-zinc-200/70 space-y-0.5">
                <div className="flex items-baseline justify-between tabular-nums">
                  <span className="text-sm @[640px]:text-base font-bold text-zinc-900">
                    {Number.isFinite(d.precipitation_sum[di]) ? `${d.precipitation_sum[di].toFixed(1)} mm` : "– mm"}
                  </span>
                  <span className="text-xs @[640px]:text-sm font-semibold text-zinc-700">{probLabel}</span>
                </div>
