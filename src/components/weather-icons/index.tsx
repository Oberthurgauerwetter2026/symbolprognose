...
export function IconSunThunder({ size, intensity = 2, ...rest }: IconProps & { intensity?: 2 | 3 | 4 }) {
  return (
    <Svg size={size} {...rest}>
      ...
      <Bolt />
      {/* Tropfen rechts vom Blitz (Bolt-BBox x 24–38), oben gehalten damit nichts abschneidet */}
...
export function IconSunSnowThunder({ size, intensity = 2, ...rest }: IconProps & { intensity?: 2 | 3 | 4 }) {
  return (
    <Svg size={size} {...rest}>
      ...
      {intensity >= 3 && <Flake x={50} y={54} size={0.95} />}
      {intensity >= 4 && <Flake x={42} y={54} size={0.95} />}
    </Svg>
  );
}
