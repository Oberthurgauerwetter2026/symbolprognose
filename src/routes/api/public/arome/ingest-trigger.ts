...
import { dispatchAromeIngest } from "@/lib/arome-dispatch.server";

/**
 * Externer Trigger-Endpoint für den AROME-HD-Ingest-Workflow.
 * Primärer Trigger: Cloudflare Worker Cron (alle 15 min).
 * Auth: Header `x-trigger-secret` == process.env.RADAR_TRIGGER_SECRET
 *       (geteilt mit Radar / EPS / Open-Meteo).
 */
...
export const Route = createFileRoute("/api/public/arome/ingest-trigger")({
  server: {
    handlers: {
      OPTIONS: async () =>
      ...
      POST: async ({ request }) => {
        ...
        const result = await dispatchAromeIngest();
        if (result.ok) {
