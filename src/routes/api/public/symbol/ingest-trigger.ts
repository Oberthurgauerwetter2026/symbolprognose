...
import { dispatchSymbolIngest } from "@/lib/symbol-dispatch.server";

/**
 * Externer Trigger-Endpoint für den Open-Meteo Symbolprognose-Workflow (phaseA).
 * Primärer Trigger ist der Cloudflare Worker Cron (cron-worker/), 4× täglich
 * ~2 h nach den Modellläufen 00/06/12/18 UTC.
 *
 * Auth: Header `x-trigger-secret` == process.env.RADAR_TRIGGER_SECRET
 * (gleiches Secret wie Radar- und EPS-Trigger).
 */
...
export const Route = createFileRoute("/api/public/symbol/ingest-trigger")({
  server: {
    handlers: {
      OPTIONS: async () =>
      ...
      POST: async ({ request }) => {
        ...
        const result = await dispatchSymbolIngest();
        if (result.ok) {
