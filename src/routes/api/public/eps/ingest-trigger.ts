...
import { dispatchEpsIngest } from "@/lib/eps-dispatch.server";

/**
 * Externer Trigger-Endpoint für den ICON-EPS-Ingest-Workflow.
 * Primärer Trigger ist der Cloudflare Worker Cron (cron-worker/).
 * GitHub-Actions `schedule:` bleibt als Backup aktiv.
 *
 * Auth: Header `x-trigger-secret` == process.env.RADAR_TRIGGER_SECRET
 * (gleiches Secret wie Radar-Trigger — vereinfacht Worker-Konfiguration).
 */
...
export const Route = createFileRoute("/api/public/eps/ingest-trigger")({
  server: {
    handlers: {
      OPTIONS: async () =>
      ...
      POST: async ({ request }) => {
        ...
        const result = await dispatchEpsIngest();
        if (result.ok) {
