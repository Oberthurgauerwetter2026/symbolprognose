...
import { dispatchOpenmeteoIngest } from "@/lib/openmeteo-dispatch.server";

/**
 * Externer Trigger-Endpoint für den Open-Meteo-Ingest-Workflow.
 * Primärer Trigger ist der Cloudflare Worker Cron (cron-worker/) alle 5 min.
 * Befüllt den R2-Cache `openmeteo/forecast.json` (phase1, phaseC), aus dem
 * der Radar-Forecast die ICON-CH1 minutely_15 Daten liest.
 *
...
export const Route = createFileRoute("/api/public/openmeteo/ingest-trigger")({
  server: {
    handlers: {
      OPTIONS: async () =>
      ...
      POST: async ({ request }) => {
        ...
        const result = await dispatchOpenmeteoIngest();
        if (result.ok) {
