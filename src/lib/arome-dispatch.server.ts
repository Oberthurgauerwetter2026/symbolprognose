 ...
 * Aufgerufen von:
 *   - Cloudflare Worker Cron (cron-worker/src/index.ts) alle 15 min
...
export async function dispatchAromeIngest(): Promise<DispatchResult> {
  ...
  const url = `https://api.github.com/repos/${repo}/actions/workflows/arome-ingest.yml/dispatches`;
  const res = await fetch(url, {
    ...
    headers: {
      ...
      "User-Agent": "lovable-arome-trigger",
